...
from lvs.tests import init_test_audio
...
def draw_curve(current_epoch):
    ...
    ax.plot(x_epoch, y_loss, label='train')
    if current_epoch == 0:
        ax.legend()
    fig.savefig(os.path.join(graph_dir, 'train.jpg'))
...
for epoch in range(epochs):
  ...
  y_loss.append(train_loss / len(training_dataset))
  draw_curve(epoch)
